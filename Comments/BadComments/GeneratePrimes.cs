 ...
 * circumference of the Earth. Also known for working on calendars with leap years and ran the library at Alexandria.
 * <p>
    ...
    public static int[] GeneratePrimes(int maxValue)
    {
        if (maxValue >= 2) // the only valid case
        {
            ...
            for (int i = 2; i < Math.Sqrt(s) + 1; i++)
            {
                ...
                {
                    for (int j = 2 * i; j < s; j += i) f[j] = false; // multiple is not prime
                }
            }
